...
// Applies skeletal animation with PBR material support
...
{
    float4x4 model;
    float4 baseColor;  // Can be used for tinting
};
...
VSOutput main(VSInput input)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (weight > 0.0)
        {
            ...
            skinnedTangent += weight * mul(input.tangent, (float3x3)bone);
        }
    }
    ...
    output.worldTangent = normalize(mul(skinnedTangent, normalMatrix));
