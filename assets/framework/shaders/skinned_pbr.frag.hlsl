...
float4 main(PSInput input) : SV_Target
{
    ...
    float4 albedo = albedoSample * baseColor;
    ...
    // Get normal (use tangent from vertex shader for proper skinned normal mapping)
    float3 N = GetNormalFromMap(input.worldNormal, input.worldTangent, input.uv);
