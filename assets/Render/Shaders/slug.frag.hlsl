...
{
	// Calculate the root eligibility code for a sample-relative quadratic B�zier curve.
...
float SlugRender(Texture2D curveData, Texture2D<uint4> bandData, float2 renderCoord, float4 bandTransform, int4 glyphData)
{
	...
	for (curveIndex = 0; curveIndex < int(hbandData.x); curveIndex++)
	{
		...
		// coordinates makes the curve relative to the sample position. The quadratic B�zier curve
		// C(t) is given by
		//
