...
/// GPU parameters for FXAA (must match fxaa.frag.hlsl cbuffer).
[CRepr]
struct FXAAParams
{
...
/// Post-process effect that applies FXAA 3.11 quality anti-aliasing.
/// Single fullscreen pass, no persistent state. Used when TAA is disabled.
public class FXAAEffect : IPostProcessEffect
{
	...
	public StringView Name => "FXAA";

	public int Priority => 310; // Anti-aliasing range, after TAA
	...
	private Result<void> CreatePipeline(IDevice device)
	{
		...
		let shaderResult = mRenderSystem.ShaderSystem.GetShaderPair("fxaa");
		...
		RGHandle depthHandle)
	{
		...
		// Only active when AA mode is FXAA
		let world = mRenderSystem?.ActiveWorld;
		if (world == null || world.AAMode != .FXAA)
			return;

		// Upload params
		var fxaaParams = FXAAParams();
		fxaaParams.TexelSizeX = 1.0f / (float)view.Width;
		fxaaParams.TexelSizeY = 1.0f / (float)view.Height;
		...
		graph.AddRenderPass("PostProcess_FXAA", scope (builder) => {
	...
	private void ExecutePass(IRenderPassEncoder encoder, ViewContext view, ITextureView inputView)
	{
		...
		BindGroupEntry[3] entries = .(
			BindGroupEntry.Buffer(/*0,*/mParamsBuffer, 0, (uint64)FXAAParams.Size),
