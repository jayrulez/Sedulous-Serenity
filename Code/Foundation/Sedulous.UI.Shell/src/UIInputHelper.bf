...
/// Polling-based keyboard input helper for routing shell keyboard state to a UIContext.
...
class UIInputHelper
{
	...
	public void ProcessKeyboardInput(Sedulous.Shell.Input.IKeyboard keyboard, UIContext context, float deltaTime)
	{
	...
	/// Call this once per frame.
	public static void ProcessMouseInput(Sedulous.Shell.Input.IMouse mouse, Sedulous.Shell.Input.IKeyboard keyboard, UIContext context)
	{
		...
		if (mouse.DeltaX != 0 || mouse.DeltaY != 0)
			context.ProcessMouseMove(mx, my, mods);
		...
		if (mouse.ScrollX != 0 || mouse.ScrollY != 0)
			context.ProcessMouseWheel(mx, my, mouse.ScrollX, mouse.ScrollY, mods);
	}

	/// Route mouse input with explicit override coordinates.
	/// Used for cross-window drag routing where global coords are converted
	/// to target-window-relative coords.
	public static void ProcessMouseInput(Sedulous.Shell.Input.IMouse mouse, Sedulous.Shell.Input.IKeyboard keyboard, UIContext context, float overrideX, float overrideY)
	{
		let mods = keyboard != null ? InputMapping.MapModifiers(keyboard.Modifiers) : Sedulous.UI.KeyModifiers.None;

		// Always send move — global position may change even if window-relative delta is zero
		context.ProcessMouseMove(overrideX, overrideY, mods);

		CheckMouseButton(mouse, context, .Left, overrideX, overrideY, mods);
		CheckMouseButton(mouse, context, .Right, overrideX, overrideY, mods);
		CheckMouseButton(mouse, context, .Middle, overrideX, overrideY, mods);

		if (mouse.ScrollX != 0 || mouse.ScrollY != 0)
			context.ProcessMouseWheel(overrideX, overrideY, mouse.ScrollX, mouse.ScrollY, mods);
	}

	private static void ForwardKeyIfPressed(Sedulous.Shell.Input.IKeyboard keyboard, UIContext context, ShellKeyCode shellKey, Sedulous.UI.KeyModifiers mods)
	{
		if (keyboard.IsKeyPressed(shellKey))
			context.ProcessKeyDown(InputMapping.MapKey(shellKey), mods);
	}

	private static void CheckMouseButton(Sedulous.Shell.Input.IMouse mouse, UIContext context, Sedulous.Shell.Input.MouseButton shellButton, float x, float y, Sedulous.UI.KeyModifiers mods)
	{
	...
	private void HandleKeyRepeat(Sedulous.Shell.Input.IKeyboard keyboard, UIContext context, Sedulous.UI.KeyModifiers mods, float deltaTime)
	{
