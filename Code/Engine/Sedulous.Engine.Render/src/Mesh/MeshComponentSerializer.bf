...
/// Custom serializer for MeshComponent.
/// Gathers data from RenderSceneModule's internal storage during write,
/// and creates mesh instances via module API during read.
class MeshComponentSerializer : IComponentSerializer
{
	public StringView TypeName => "MeshComponent";
	...
	public SerializationResult Write(Scene scene, Serializer s, Dictionary<uint32, int32> entityIndexMap)
	{
		...
		// Gather mesh instances that correspond to active entities
		let entries = scope List<(int32 entityIdx, MeshComponentData data)>();
		for (let instance in ref renderModule.MeshInstances)
		{
			if (!instance.Active)
				continue;
			if (entityIndexMap.TryGetValue(instance.Entity.Index, let idx))
			{
				var data = MeshComponentData();
	...
	public SerializationResult Read(Scene scene, Serializer s, List<EntityId> loadedEntities)
	{
		...
		for (int32 i = 0; i < count; i++)
		{
			...
			if (entityIdx >= 0 && entityIdx < loadedEntities.Count && renderModule != null)
			{
				...
				renderModule.CreateMeshFromRef(entity, data.MeshRef, data.Enabled);

				for (int32 m = 0; m < data.MaterialRefs.Count; m++)
					renderModule.SetMeshMaterialRef(entity, m, data.MaterialRefs[m]);
			}
