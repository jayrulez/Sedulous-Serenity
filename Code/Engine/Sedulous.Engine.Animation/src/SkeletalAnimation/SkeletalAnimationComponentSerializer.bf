...
/// Custom serializer for SkeletalAnimationComponent.
/// Gathers data from AnimationSceneModule's internal storage during write,
/// and creates skeletal animation instances via module API during read.
class SkeletalAnimationComponentSerializer : IComponentSerializer
{
	public StringView TypeName => "SkeletalAnimationComponent";
	...
	public SerializationResult Write(Scene scene, Serializer s, Dictionary<uint32, int32> entityIndexMap)
	{
		...
		let entries = scope List<(int32 entityIdx, SkeletalAnimationComponentData data)>();
		for (let instance in ref animModule.SkeletalAnimInstances)
		{
			if (!instance.Active)
				continue;
			if (entityIndexMap.TryGetValue(instance.Entity.Index, let idx))
			{
				var data = SkeletalAnimationComponentData();
				data.SkeletonRef = instance.SkeletonRef;
				data.AnimationClipRef = instance.AnimationClipRef;
	...
	public SerializationResult Read(Scene scene, Serializer s, List<EntityId> loadedEntities)
	{
		...
		for (int32 i = 0; i < count; i++)
		{
			...
			if (entityIdx >= 0 && entityIdx < loadedEntities.Count && animModule != null)
			{
				...
				animModule.CreateSkeletalAnimation(entity, data.SkeletonRef, data.AnimationClipRef, data.Playing, data.Loop);
			}
