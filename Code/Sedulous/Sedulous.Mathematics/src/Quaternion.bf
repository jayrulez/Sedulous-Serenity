...
/// where w = cos(theta/2).
/// </summary>
struct Quaternion : IEquatable<Quaternion>, IInterpolatable<Quaternion>, IEquatable, IHashable
{
    /// <summary>
    ...
    /// <returns>The resulting <see cref="Quaternion"/>.</returns>
    public static Quaternion operator *(float factor, Quaternion quaternion)
    {
        Quaternion result;
	...
	public static Quaternion CreateFromRotationMatrix(Matrix matrix)
	{
	    ...
	    else
	    {
	        ...
	        {
	            var s = (float)Math.Sqrt(1.0f + matrix.M11 - matrix.M22 - matrix.M33);
	            var invS = 0.5f / s;
	        ...
	        {
	            var s = (float)Math.Sqrt(1.0f + matrix.M22 - matrix.M11 - matrix.M33);
	            var invS = 0.5f / s;
	        ...
	        {
	            var s = (float)Math.Sqrt(1.0f + matrix.M33 - matrix.M11 - matrix.M22);
	            var invS = 0.5f / s;
	...
	/// <param name="quaternion2">The <see cref="Quaternion"/> to the right of the multiplication operator.</param>
	/// <param name="result">The resulting <see cref="Quaternion"/>.</param>
	public static void Multiply(in Quaternion quaternion1, in Quaternion quaternion2, out Quaternion result)
	{
	    var q1x = quaternion1.X;
	    var q1y = quaternion1.Y;
	    var q1z = quaternion1.Z;
	...
	/// <param name="factor">The scaling factor by which to multiply the quaternion.</param>
	/// <returns>The resulting <see cref="Quaternion"/>.</returns>
	public static Quaternion Multiply(Quaternion quaternion, float factor)
	{
	    Quaternion result;
	...
	/// <param name="factor">The scaling factor by which to multiply the quaternion.</param>
	/// <param name="result">The resulting <see cref="Quaternion"/>.</param>
	public static void Multiply(in Quaternion quaternion, float factor, out Quaternion result)
	{
	    result.X = quaternion.X * factor;
	    result.Y = quaternion.Y * factor;
	    result.Z = quaternion.Z * factor;
	...
	public static Quaternion Lerp(Quaternion source, Quaternion target, float amount)
	{
	    ...
	    var lengthInv = 1.0f / (float)Math.Sqrt(lengthSquared);
	...
	public static void Lerp(in Quaternion source, in Quaternion target, float amount, out Quaternion result)
	{
	    var t = amount;
	    var t1 = 1.0f - amount;
	...
	public static Quaternion Slerp(Quaternion source, Quaternion target, float amount)
	{
	    ...
	    var s1 = default(float);
	    var s2 = default(float);
	    ...
	    else
	    {
	        ...
	        s2 = flip ? -(float)Math.Sin(amount * omega) * invSinOmega : (float)Math.Sin(amount * omega) * invSinOmega;
	    }
	...
	/// <param name="amount">The relative weight of the target quaternion in the interpolation.</param>
	/// <param name="result">The interpolated quaternion.</param>
	public static void Slerp(in Quaternion source, in Quaternion target, float amount, out Quaternion result)
	{
	    var flip = false;
	...
	public static Quaternion Normalize(Quaternion quaternion)
	{
	    ...
	    var inverseMagnitude = 1f / (float)Math.Sqrt(magnitude);
	...
	public static void Concatenate(in Quaternion quaternion1, in Quaternion quaternion2, out Quaternion result)
	{
	    var q1x = quaternion2.X;
	    var q1y = quaternion2.Y;
	    var q1z = quaternion2.Z;
    ...
    {
        Slerp(this, target, t, var result);
        return result;
    }
    ...
    /// </summary>
    public static Quaternion Identity { get; } = Quaternion(0, 0, 0, 1);
