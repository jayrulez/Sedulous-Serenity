...
public function float EasingFunction(float t);
...
public static class Easings
{
    ...
    {
        return -1f * (float)Math.Cos(t * (Math.PI_f / 2.0)) + 1f;
    };
    ...
    {
        return -0.5f * ((float)Math.Cos(Math.PI_f * t) - 1f);
    };
    ...
    public static readonly EasingFunction EaseInOutElastic = (t) =>
    {
        ...
        {
            t -= 1f;
            return -0.5f * (float)(Math.Pow(2, 10 * t) * Math.Sin((t - s) * (2.0 * Math.PI_f) / p));
        }
        t -= 1f;
        return (float)(Math.Pow(2, -10 * t) * Math.Sin((t - s) * (2.0 * Math.PI_f) / p) * 0.5) + 1f;
    };
