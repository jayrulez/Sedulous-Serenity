...
class RenderMaterialsApp : Application
{
	...
	private void CreateLights()
	{
		...
		// Two fill lights
		mPointLights.Add(mWorld.CreatePointLight(.(-4, 3, 4), .(0.8f, 0.9f, 1.0f), 5.0f, 10.0f));
		mPointLights.Add(mWorld.CreatePointLight(.(4, 3, -4), .(1.0f, 0.9f, 0.8f), 5.0f, 10.0f));
	}
		...
		if (ModelMeshConverter.ConvertToSkinnedMesh(modelMesh, skin) case .Ok(var convResult))
		{
			...
			if (mRenderSystem.ResourceManager.UploadMesh(skinnedMesh) case .Ok(let gpuHandle))
			{
				...
				if (mRenderSystem.ResourceManager.CreateBoneBuffer(boneCount) case .Ok(let boneHandle))
				{
					...
					// Position fox to the right of the sphere grid
					...
					if (let proxy = mWorld.GetSkinnedMesh(mFoxProxy))
					{
						...
						proxy.SetTransformImmediate(Matrix.CreateScale(0.03f) * Matrix.CreateTranslation(.(6, -0.5f, 0)));
						proxy.Flags = .DefaultOpaque;
					}
	...
	private void UpdateDebugDrawing(float dt)
	{
		...
		// Ground grid
		mOverlayFeature.AddGrid(.(0, -0.49f, 0), 20, 10, .(128, 128, 128, 80));
	}
