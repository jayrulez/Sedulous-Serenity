...
elseif($option -eq "build")
{
    ...
    else
    {
        ...
        if($args[1] -eq "DEBUG" -or $args[1] -eq "ALL")
        {
			...
			copy-item "./build/Debug/nuklear.lib" ($targetPath + "nuklear.lib") -Force
			copy-item "./build/Debug/nuklear.pdb" ($targetPath + "nuklear.pdb") -Force
			$built = $true
        }
		...
		if($args[1] -eq "RELEASE" -or $args[1] -eq "ALL")
        {
			...
			copy-item "./build/Release/nuklear.lib" ($targetPath + "nuklear.lib") -Force
			$built = $true
        }
