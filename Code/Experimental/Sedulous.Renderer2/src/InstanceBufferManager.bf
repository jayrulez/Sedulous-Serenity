...
public class InstanceBufferManager
{
	...
	{
		using (SProfiler.Begin("InstanceBufferManager.Initialize"))
		{
			mDevice = device;
	...
	{
		using (SProfiler.Begin("InstanceBufferManager.Shutdown"))
		{
			if (mDevice != null)
			{
				for (int32 i = 0; i < RenderConfig.FrameBufferCount; i++)
	...
	{
		using (SProfiler.Begin("InstanceBufferManager.Upload"))
		{
			if (!mInitialized)
				return 0;
