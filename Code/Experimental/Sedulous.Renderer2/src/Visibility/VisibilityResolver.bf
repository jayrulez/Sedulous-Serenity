...
public struct VisibleStaticMesh
{
	/// Handle to the mesh proxy.
	public StaticMeshProxyHandle Handle;
...
public class VisibilityResolver
{
	...
	private List<VisibleStaticMesh> mVisibleStaticMeshes = new .() ~ delete _;
	...
	public Span<VisibleStaticMesh> VisibleStaticMeshes => mVisibleStaticMeshes;
	...
	public int32 VisibleCount => (int32)(mVisibleStaticMeshes.Count + mVisibleSkinnedMeshes.Count);
	...
	/// Clears previous results and performs fresh culling.
	public void Resolve(RenderWorld world, ViewContext viewCtx, SortMode sortMode = .FrontToBack)
	{
		mStats = .();

		mCuller.ResetStats();
		mCuller.SetFrustum(viewCtx.ViewProjectionMatrix);
	...
	public void ResolveAccumulate(RenderWorld world, ViewContext viewCtx, SortMode sortMode = .FrontToBack)
	{
		mCuller.ResetStats();
		mCuller.SetFrustum(viewCtx.ViewProjectionMatrix);
	...
	private void AccumulateStaticMeshes(RenderWorld world, Vector3 cameraPos)
	{
		...
		HashSet<uint32> existing = scope .((int32)mVisibleStaticMeshes.Count);
		for (let vm in mVisibleStaticMeshes)
	...
	private void SortStaticMeshes(SortMode mode)
	{
		switch (mode)
		{
		case .None:
		...
		case .FrontToBack:
			mVisibleStaticMeshes.Sort(scope (a, b) => a.DistanceSq <=> b.DistanceSq);

		case .BackToFront:
			mVisibleStaticMeshes.Sort(scope (a, b) => b.DistanceSq <=> a.DistanceSq);

		case .ByMaterial:
			mVisibleStaticMeshes.Sort(scope (a, b) => a.SortKey <=> b.SortKey);
		}
	}
...
public struct VisibilityStats
{
	...
	public int32 VisibleStaticMeshCount;

	/// Total number of static meshes in the world.
	public int32 TotalStaticMeshCount;
	...
	public float StaticMeshCullPercentage => TotalStaticMeshCount > 0
		? (float)(TotalStaticMeshCount - VisibleStaticMeshCount) / (float)TotalStaticMeshCount * 100.0f
		: 0.0f;
}
