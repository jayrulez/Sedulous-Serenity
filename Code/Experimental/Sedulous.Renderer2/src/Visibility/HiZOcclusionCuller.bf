...
public class HiZOcclusionCuller : IDisposable
{
	...
	public void CullTwoPhase(
		...
		List<StaticMeshProxyHandle> outVisibleHandles)
	{
		...
		List<StaticMeshProxyHandle> frustumVisible = scope .();
		frustumCuller.CullStaticMeshes(world, frustumVisible);
		...
		List<StaticMeshProxyHandle> handleMapping = scope .(objectCount); // Maps index -> handle

		for (int32 i = 0; i < objectCount; i++)
		{
			let handle = frustumVisible[i];
			if (let proxy = world.GetStaticMesh(handle))
			{
		...
		List<StaticMeshProxyHandle> outVisibleHandles)
	{
		...
		int32 objectCount = (int32)Math.Min(frustumVisible.Count, (int)mMaxObjects);
		List<BoundingBox> bounds = scope .(objectCount);
		List<StaticMeshProxyHandle> handleMapping = scope .(objectCount);
